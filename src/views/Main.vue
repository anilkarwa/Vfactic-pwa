            ...
            <v-flex xs8 sm4 mt-5 pt-5>
              <v-card>
                ...
                <v-list subheader three-line>
                  <v-list-tile v-for="value in companyList" :key="value.gdID" style="cursor: pointer"
                    @click="readCompanyDetials(value)">
                    <v-list-tile-content>
            ...
            <v-container grid-list-md>
              <v-layout wrap>
                <v-flex xs12 sm12 md12>
                  <v-text-field label="Server *" v-model="dataBaseDetails.server"
                    hint="example: server ip or domain name" persistent-hint required></v-text-field>
                </v-flex>
                <v-flex xs12 sm12 md12>
                  <v-text-field label="DataBase Name *" v-model="dataBaseDetails.databaseName"
                    hint="example: database which you wanna connect" persistent-hint required></v-text-field>
                </v-flex>
                <v-flex xs12 sm12 md12>
                  <v-text-field label="User Name *" v-model="dataBaseDetails.userName"
                    hint="example: username for database connection" persistent-hint required></v-text-field>
                </v-flex>
                <v-flex xs12 sm12 md12>
                  <v-text-field label="Password *" v-model="dataBaseDetails.password"
                    hint="example: password for database connection" persistent-hint required></v-text-field>
                </v-flex>
              </v-layout>
            </v-container>
