      ...
      <v-data-table :headers="headers" :items="listOfPostDocEntries" class="elevation-1">
         <template v-slot:items="props">
            <td class="text-xs-center" style="cursor:pointer">
              <v-icon small class="mr-2" @click="postDocDialog=true;selectedPostDocId = props.item.postDocId;getPostDocData();">view_headline</v-icon>
            </td>
    ...
    <v-dialog v-model="postDocDialog" fullscreen hide-overlay transition="dialog-bottom-transition">
        ...
        <v-container class="spaceFromTop" fluid grid-list-xl>
            ...
            <v-menu  reff="datePickerModal" v-model="datePickerModal" :close-on-content-click="false" lazy transition="scale-transition" offset-y
                ...
                <v-text-field v-model="dateFormatted"  label="From Date" persistent-hint prepend-icon="event"  @blur="date = parseDate(dateFormatted)"  v-on="on"   ></v-text-field>
                </template>
                <v-date-picker v-model="date" no-title @input="datePickerModal = false"></v-date-picker>
                </v-menu>
            </v-flex>
            <v-flex xs12 sm4 d-flex>
                <v-menu  reff="datePickerModal" v-model="toDatePickerModal" :close-on-content-click="false" lazy transition="scale-transition" offset-y
                full-width max-width="290px"  min-width="290px" >
                <template v-slot:activator="{ on }">
                <v-text-field v-model="toDateFormatted"  label="To Date" persistent-hint prepend-icon="event"  @blur="date2 = parseDate(toDateFormatted)"  v-on="on"   ></v-text-field>
                </template>
        ...
        <v-card>
            <v-card-title> 
                    ...
                    <v-data-table  :headers="selectedPostDocHeader" :items="selectedPostDocData"
                    ...
                    <template v-slot:items="props">
                        ...
                        <td> <v-icon samll class="mr-2" @click="editPostDocRow(props.item)">edit</v-icon></td>
...
<!-- Edit Post doc row modal -->
    ...
    <v-dialog v-model="detailModal" scrollable persistent max-width="450px">
        ...
        <v-card-text>
            ...
            <v-layout wrap>
              <v-flex xs12>
                <vue-form-generator id="Form-generator-css" ref="vfAddDetail" :schema="postDocDynamicSchema" :model="postDocDynamicModal" :options="formOptions" ></vue-form-generator>
              </v-flex>
            </v-layout>
            </v-container>
