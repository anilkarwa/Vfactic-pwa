    ...
    <v-app id="inspire">
          ...
          <template v-slot:items="props">
            <td class="text-xs-center">
              <v-icon small class="mr-2"
                @click="loadPendingAuthDocForSelectedDoc(props.item.docId, props.item.docName, props.item.hdrTableName, props.item.docPrefix)">view_headline</v-icon>
            </td>
      ...
      <v-dialog v-model="authDialog" fullscreen hide-overlay transition="dialog-bottom-transition">
        ...
        <v-card>
          ...
          <v-data-table v-model="selected" :headers="authDocHeaders" :items="authDocTableData" select-all
            :item-key="docPrefix + 'ID'" class="elevation-1" hide-actions>
                      ...
                      <v-card-text>
                        <v-container fluid grid-list-x>
                          <v-layout row justify-space-between v-if="selectedRowData.hasOwnProperty('mainData')">
                            <v-flex xs12 sm7 md7>
                              <v-layout row>
                                <v-flex xs3 sm3 md3><b> {{ docPrefix }}NO :
                                  </b>{{ selectedRowData.mainData[docPrefix + "NO"] }}
                                </v-flex>
                                <v-flex xs2 sm2 md3> <b>{{ docPrefix }}Date :</b>
                                  {{ selectedRowData.mainData[docPrefix + "Date"] }}</v-flex>
                              </v-layout>
                              ...
                              <v-layout row>
                                <v-flex xs4 sm4 md4>
                                  ...
                                  <p>{{ selectedRowData.mainData.partyAddress3 }}
                                    {{ selectedRowData.mainData.partyAddress4 }}</p>
                                  <p>{{ selectedRowData.mainData.city }} {{ selectedRowData.mainData.PinCode }}
                                    {{ selectedRowData.mainData.state }} {{ selectedRowData.mainData.country }}</p>
                                </v-flex>
                              </v-layout>
                            ...
                            <v-flex xs12 sm4 md4>
                              <template
                                ...
                                v-for="key in Object.keys(selectedRowData.headerFields.dynamicFieldModal.modal[0])">
                                <p :key="key"><b>{{ key }} : </b>
                                  {{ selectedRowData.headerFields.dynamicFieldModal.modal[0][key] }} </p>
                              </template>
                            </v-flex>
                          </v-layout>
                        </v-container>
                    ...
                    <v-card>
                      <v-card-text>
                        <v-container fluid grid-list-xl>
                          <v-data-table v-model="selected" :headers="detailSectionHeader"
                            :items="detailSectionTableData" select-all item-key="SLNO" class="elevation-1" hide-actions>
                            <template v-slot:headers="props">
                              <tr>
                            ...
                            <template v-slot:items="props">
                              <tr>
                                <template v-for="header of detailSectionHeader">
                                  <td class="text-xs-left" :key="header.text"
                                    v-if="header != 'ITEMID' && header != 'UOMID' && header != 'ITEMSLNO'">{{
          props.item[header.text] }}</td>
                                </template>
                              </tr>
                            </template>
                          </v-data-table>
                      ...
                      <v-card-text>
                        <v-container fluid grid-list-xl>
                          <v-layout row justify-space-between v-if="selectedRowData.hasOwnProperty('mainData')">
                            <v-flex xs12 sm6 md6>
                              <template
                                v-for="key in Object.keys(selectedRowData.footerFields.dynamicFieldModal.modal[0])">
                                <p :key="key"><b>{{ key }} : </b>
                                  {{ selectedRowData.footerFields.dynamicFieldModal.modal[0][key] }} </p>
                              </template>
                            </v-flex>
                            <v-flex xs12 sm4 md4>
                              <template
                                
                                v-for="key in Object.keys(selectedRowData.totalFields.dynamicFieldModal.modal[0])">
                                <p :key="key"><b>{{ key }} : </b>
                                  {{ selectedRowData.totalFields.dynamicFieldModal.modal[0][key] }} </p>
                              </template>
                            </v-flex>
                          </v-layout>
                        </v-container>
