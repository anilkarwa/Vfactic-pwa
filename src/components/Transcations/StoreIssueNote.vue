                            ...
                            <v-flex xs12 sm7 md7>
                              ...
                              <v-layout row>
                                <v-flex xs3 sm3 md3> <v-select v-model="selectedPrefix" :items="prefixDropdown"
                                    label="Prefix" @change="getDocNumber()" item-text="prefix"
                                    item-value="prefix"></v-select></v-flex>
...
export default {
  ...
  methods: {
    ...
    loadPartDocTableData() {
      ...
      httpClient({
        method: 'GET',
        url: `${process.env.VUE_APP_API_BASE}StoreIssueNote?docID=${docID}`
      })
        .then((result) => {
    ...
    editSelectedPartyDocTranscation(params) {
      ...
      httpClient({
        method: 'GET',
        url: `${process.env.VUE_APP_API_BASE}StoreIssueNote?selectedID=${this.selectedID}&docID=${docID}`
      })
        .then((result) => {
    ...
    loadSchemaForNewPartyDocTranscation() {
      ...
      httpClient({
        method: 'GET',
        url: `${process.env.VUE_APP_API_BASE}StoreIssueNote?docID=${docID}&type=0`
      })
        .then((result) => {
    ...
    savePartyDocHDRTableData() {
      ...
      if (this.validateStaticFields() && this.isDynamicHeaderFormValid && this.isDynamicFooterFormValid && this.isDynamicTotalFormValid && this.detailSectionData.length > 0) {
        ...
        httpClient({
          method: 'POST',
          url: `${process.env.VUE_APP_API_BASE}StoreIssueNote`,
          data: updateParams
        })
          .then((result) => {
    ...
    savePartyDocDTLTableData(selectedId) {
      ...
      httpClient({
        method: 'POST',
        url: `${process.env.VUE_APP_API_BASE}StoreIssueNote?query=${query}&docID=${docID}&selectedId=${selectedId}`,
      })
        .then((result) => {
        ...
        .then((result) => {
          ...
          if (result.data.tableData.length > 0) {
            ...
            loadPendingHeadersKey.forEach(element => {
              if (element != "ITEMID" && element != this.dependentPrefix + "ID" && element != "ITEMSLNO" && element != "ITEMROWID" && element != "CONVFACT" && element != "CODE" && element != "NAME") {
