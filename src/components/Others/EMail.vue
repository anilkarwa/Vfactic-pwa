      ...
      <v-card ref="form">
        <v-card-title aria-required="TO">
          ...
          <v-dialog v-model="dialog" scrollable max-width="600px" margin-left="1014px">
            ...
            <v-card>
              ...
              <v-list three-line>
                ...
                <v-divider></v-divider>
                <v-data-table :headers="headers" :search="tableSearch" :items="partypersonalTableData"
                  :pagination.sync="pagination" class="elevation-1">
                  <template v-slot:items="props">
                    <td class="text-xs-right"> <v-checkbox v-model="to" v-bind:value="props.item.email">
                      </v-checkbox></td>
          ...
          <v-flex xs12 sm12 md12>
            <v-text-field ref="to" v-model="to" :rules="[() => !!to || 'This field is required']"
              :error-messages="errorMessages" label="TO:" placeholder="Mail Address" required></v-text-field>
          </v-flex>
        </v-card-title>
        <v-card-title>
          <v-flex xs12 sm12 md12>
            <v-text-field ref="subject" v-model="subject" :rules="[() => !!subject || 'This field is required']"
              :error-messages="errorMessages" label="SUBJECT:" placeholder="Enter Subject..." required></v-text-field>
          </v-flex>
        </v-card-title>
        <v-card-title>
          <v-flex xs12 sm12 md12>
            <v-textarea ref="message" v-model="message" :rules="[() => !!message || 'This field is required']"
              :error-messages="errorMessages" name="input-7-1" label="Message..."
              value="Progressive Web App(PWA)....."></v-textarea>
          </v-flex>
        </v-card-title>
