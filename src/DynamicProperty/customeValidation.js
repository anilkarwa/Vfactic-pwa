  ...
  if (tempSchema.length > 0) {
    tempSchema.forEach((element) => {
      ...
      if (element.requiredCondtion != "") {
        required = isRequired(
          model,
          element.requiredCondtion,
          element.requiredOperator,
          element.requiredCompareValue
        );
...
function isRequired(model, conditionsOperand, operator, compareValue) {
  ...
  if (requiredCondition != "") {
    if (
      requiredCondition.substring(0, 1) == "#" ||
      requiredCondition.substring(0, 1) == "("
    ) {
      ...
      switch (operator.trim()) {
        case "=":
          return model && requiredCondition == compareValue.trim();
          break;
        case "<":
          return model && requiredCondition < compareValue.trim();
          break;
        ...
        case ">=":
          ...
          break;
        case "<>":
          return model && requiredCondition != compareValuee.trim();
          break;
      }
    }
  }
